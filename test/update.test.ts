...
vi.mock('node:child_process', () => ({
  ...
  spawn: (cmd: string, args: string[]) => {
    ...
    queueMicrotask(() => handlers.exit?.(h.stepExit));
    return {
      pid: 1,
      unref() {},
...
describe('restart', () => {
  it('starts a replacement process when nothing else will', async () => {
    const { restart } = await freshUpdate();
    restart();
    expect(h.spawns.at(-1)?.cmd).toBe('/bin/sh');
