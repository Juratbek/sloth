...
export default function StepRunner({
  ...
  const runnerRoot = root ?? (repo ? `~/sloth-runner/${repo.split('/')[1]}` : '');
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Runner root" hint="The local checkout the sessions run from. Transcripts are read from the Claude Code project folder derived from it.">
        <TextInput value={runnerRoot} onChange={setRoot} placeholder="~/sloth-runner/repo" />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Max alive sessions" hint="active plus parked, before the watcher stops picking up">
      ...
      <div className="flex gap-2">
        ...
        <Button variant="primary" disabled={!ready} onClick={() => onContinue({ repo, runnerRoot, ...caps })}>
          Continue
        </Button>
      </div>
    </div>
