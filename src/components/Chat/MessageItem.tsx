...
function Footer({ message }: { message: Message }) {
  ...
  return (
    <div className="flex gap-3 pt-1 text-[11px] text-zinc-500">
        ...
        if (b.type === 'thinking')
          return (
            <details key={i} className="max-w-4xl">
              <summary className="text-xs text-zinc-500 hover:text-zinc-400">thinking…</summary>
              <p className="mt-1 border-l border-zinc-800 pl-3 text-xs whitespace-pre-wrap text-zinc-400 italic">
                {b.text}
              </p>
            </details>
          );
