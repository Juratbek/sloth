...
/** Keyed by the issue — `owner/name#12` — since two repositories both have an issue 12. */
const live = new Map<string, Live>();
const prChecked = new Map<string, number>();
const warned = new Set<string>();

const stateFile = (issue: IssueRef) => path.join(issueDir(issue), 'preview-state.json');
const prOf = (issue: IssueRef) => Number(/\/pull\/(\d+)/.exec(stateOf(issueDir(issue)).pr ?? '')?.[1]) || undefined;

/** The preview Sloth keeps for an issue, if any — what the monitor shows. */
export function previewState(issue: IssueRef): PreviewState | undefined {
  try {
    return { ...(JSON.parse(readFile(stateFile(issue)) ?? '') as PreviewState), repo: issue.repo };
  } catch {
    return undefined;
  }
}
...
export function previewLink(issue: IssueRef): string | undefined {
 ...
 */
async function announce(issue: IssueRef, url: string): Promise<void> {
  const s = previewState(issue);
  ...
  log(`preview ${label(issue)} up at ${url} until ${when(s.expiresAt)}${s.pr ? ` (PR #${s.pr})` : ''}`);
  broadcast();
}
 ...
 */
async function openTunnel(issue: IssueRef, upstream: string, key: string): Promise<void> {
  ...
  child.on('error', (e) => log(`preview ${label(issue)}: ${cmd} failed: ${e.message}`));
  child.on('exit', (code) => {
    if (live.get(refKey(issue)) !== entry) return;
    live.delete(refKey(issue));
    proxy.close();
    log(`preview ${label(issue)}: ${cmd} exited with ${code} — a new link comes with the next tick`);
  });
}
...
async function begin(issue: IssueRef): Promise<void> {
  ...
  if (!cfg().previewHours) {
    log(`preview ${label(issue)} skipped: previewHours is 0 — cleaning the run up`);
    await cleanup(issue);
    return;
  }
  if (isDry()) {
    log(`dry-run: would put a tunnel in front of ${p.url} for ${label(issue)}`);
    return;
  }
  const now = nowSec();
  const s: PreviewState = { issue: issue.number, repo: issue.repo, pr: prOf(issue), key: newKey(), startedAt: now, expiresAt: now + cfg().previewHours * 3600 };
  write(stateFile(issue), JSON.stringify(s));
  await openTunnel(issue, p.url, s.key);
}

async function prClosed(issue: IssueRef, s: PreviewState): Promise<boolean> {
  if (!s.pr || Date.now() - (prChecked.get(refKey(issue)) ?? 0) < PR_CHECK_MS) return false;
  prChecked.set(refKey(issue), Date.now());
  const r = await gh(['pr', 'view', String(s.pr), '--repo', issue.repo, '--json', 'state', '--jq', '.state']);
...
export async function stopPreview(issue: IssueRef, reason: string): Promise<void> {
  ...
  await cleanup(issue);
  log(`preview ${label(issue)} stopped: ${reason}`);
  broadcast();
}
