 ...
 * the tunnel's (or `publicUrl`'s) address. Without one there is nothing anybody off this machine could
 * open, so nothing is written. The link is bare on purpose — a browser signed in through the QR opens
 * it, and the single-use code stays out of a thread everyone with read access can see.
 *
