...
import { forgetPause, pausedForDeadline, resumeRun } from './pressure';
...
export async function reap(): Promise<void> {
  for (const r of runDirs()) {
    ...
    const deadline = Math.max(launchedAt(dir) + pausedForDeadline(dir, budget) + waitedForDeadline(dir) + budget, answeredAt(dir) + ANSWER_MINUTES * 60);
